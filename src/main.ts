...
async function main() {
	...
	switch (process.argv[2]) {
		case "generate":
			await generateSalesTranscript(client, process.argv[3]);
			break;
		case "summarize":
			await summarizeTranscript(client, process.argv[3]);
			break;
		case "answer":
			await answerQuery(client, process.argv[3], process.argv[4]);
			break;
		default:
